...
# from interview_agent.utilities import log
...
from interview_agent.mongodb_handler.base import DatabaseHandler
from interview_agent.mongodb_handler.response import RESPONSE_TYPE
...
from interview_agent.mongodb_handler.utils.mongodb_parser import MongodbParser
from interview_agent.mongodb_handler.utils.mongodb_query import MongoQuery
from interview_agent.mongodb_handler.utils.mongodb_render import MongodbRender
...
class MongoDBHandler(DatabaseHandler):
    ...
    def connect(self) -> MongoClient:
        ...
        try:
            ...
            logger.error(
                f"Invalid URI provided for MongoDB connection: {invalid_uri_error}!"
            )
            raise
    ...
    def subscribe(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            table_name (str): The name of the collection to subscribe to.
        ...
        while True:
            ...
            if res["operationType"] == "update":
                if columns is not None:
                    updated_columns = set(
                        res["updateDescription"]["updatedFields"].keys()
                    )
                    if not set(columns) & set(updated_columns):
                        # Do nothing.
                        continue
    ...
    def check_connection(self) -> StatusResponse:
        ...
        try:
            ...
            logger.error(
                f"Unknown error connecting to MongoDB {self.database}, {unknown_error}!"
            )
    ...
    def native_query(self, query: Union[str, dict, MongoQuery]) -> Response:
        """
        ...
        try:
            ...
            logger.error(
                f"Error running query: {query} on {self.database}.{collection}!"
            )
    ...
    def get_columns(self, table_name: str) -> Response:
        ...
        Args:
            table_name (str): The name of the table (collection) for which to retrieve column (field) information.
