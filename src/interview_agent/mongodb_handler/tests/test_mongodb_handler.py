...
import pytest
from interview_agent.mongodb_handler.mongodb_handler import MongoDBHandler
...
def seed_db():
    ...
    with open(
        "mindsdb/integrations/handlers/mongodb_handler/tests/seed.json", "r"
    ) as f:
        json.load(f)
    conn.close()
